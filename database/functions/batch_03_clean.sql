CREATE OR REPLACE FUNCTION create_recording(
    p_hadith_id UUID,
    p_file_path TEXT,
...
BEGIN
    ...
    -- 10. Check unique constraint (ALG-004: one recording per user per hadith)
    IF EXISTS (SELECT 1 FROM recordings WHERE hadith_id = p_hadith_id AND user_id = v_user_id) THEN
        RETURN jsonb_build_object(
            'success', false, 'data', NULL,
            'message', 'لديك تسجيل لهذا الحديث — استخدم الاستبدال',
            'errors', jsonb_build_array('duplicate_recording')
        );
    END IF;

    -- 11. Insert new recording
